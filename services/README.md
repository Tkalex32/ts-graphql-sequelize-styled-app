...
`yarn` or `npm` will install all necessary Node dependencies.
...
or just in one line (for windows command prompt, terminal)

docker run -p 0.0.0.0:7999:3306 --name gsd-db -e MYSQL_ROOT_PASSWORD=password -e MYSQL_USER=gsd-dev -e MYSQL_PASSWORD=password -e MYSQL_DATABASE=gsd -d mysql:5.7.20
